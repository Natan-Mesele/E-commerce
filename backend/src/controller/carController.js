...
module.exports = {
  ...
  async getCarsByCategory(req, res) {
    ...
    try {
      ...
      if (category) {
        ...
        cars = await carService.getCarsByCategory(category);
      } else {
        // If no category is provided, return all cars
        cars = await carService.getAllCars();
      }
  
      if (cars.length === 0) {
        return res.status(404).json({ message: `No cars found for category: ${category || 'Uncategorized'}` });
      }
